    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        elseif ($step === 'reset' && isset($_POST['new_pin'], $_POST['confirm_pin'])) {
            ...
            if (!preg_match('/^\d{4}$/', $newPin)) {
                ...
                $res = db()->prepare(
                    "SELECT id, resident_name, address, resident_erfno, occupant_code, phone,
      ...
      <div class="login-card">
        ...
        <form method="POST" action="resident.php?action=login" id="loginForm">
          ...
          <div class="form-group" id="occupantGroup" style="display:none;">
            ...
            <select id="occupantSelect"
                    style="font-size:1rem;padding:10px 12px;">
              <option value="A">A — Loading…</option>
            </select>
          </div>
          <!-- Single source of truth for occupant code submitted with the form -->
          <input type="hidden" name="occupant_code" id="occupantFinal" value="A">
          <div class="form-group">
        ...
        function loadOccupants(erf) {
          ...
          const final = document.getElementById('occupantFinal');
            ...
            fetch('resident.php?action=get_occupants&erf=' + encodeURIComponent(erf))
              ...
              .then(data => {
                ...
                if (data.length === 1) {
                  ...
                  // Keep the hidden field in sync with whatever is selected
                  sel.onchange = () => { final.value = sel.value; };
                  final.value = sel.value; // initialise to first option
                }
              })
              .catch(() => {
                grp.style.display = 'none';
...
if ($action === 'menu') {
    ...
    renderHeader('🏠 ' . $rerf . ($_SESSION['resident_occupant'] ?? 'A') . ' — ' . $rname, 'logout.php');
    ?>
    <div class="container">
      <?= getFlash() ?>
      <div class="menu-grid">
        ...
        <a href="comms_archive.php" class="menu-btn">
       ...
       <a href="resident.php?action=reset" class="menu-btn">
        ...
        <a href="survey_respond.php?action=list" class="menu-btn">
         <span class="icon">📋</span>Surveys
        </a>
        </a>
...
function residentGrantAccess(array $res): void {
    ...
    $_SESSION['resident_occupant']  = $res['occupant_code'] ?? 'A';
