...
if ($action === 'notify' && $surveyId > 0) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $audience = ($_POST['audience'] ?? 'comms_contacts') === 'vote_eligible_owners'
            ? 'vote_eligible_owners' : 'comms_contacts';
        ...
        // SITE_URL is defined as https://gemb.co.za/comms in config.php
        // so this resolves to https://gemb.co.za/comms/survey_respond.php
        $sent = 0; $failed = 0;
    ...
    $allContactCount = commsRecipientCount('comms_contacts');
        ...
        <form method="POST">
          ...
          <div class="form-group">
            ...
            <select name="audience" id="audienceSelect" onchange="document.getElementById('meetingGroup').style.display = this.value === 'vote_eligible_owners' ? 'block' : 'none';">
              <option value="comms_contacts">All contacts (<?= $allContactCount ?>)</option>
...
if ($action === 'close' && $surveyId > 0) {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($upd->rowCount() > 0) {
            // Purge this survey's device locks only — type='survey' scoping is
            // critical here since device_response_locks also holds 'vote' rows,
            // which must never be touched.
            db()->prepare("DELETE FROM device_response_locks WHERE type='survey' AND target_id=?")
                ->execute([$surveyId]);
            setFlash('success', 'Survey closed. No further responses will be accepted, and device tokens for this survey have been purged.');
        } else {
...
if ($action === 'delete' && $surveyId > 0) {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (!$row) {
            ...
            // Defensive backstop — close already purges these, but cover the
            // edge case of a draft survey deleted directly. type='survey' only.
            db()->prepare("DELETE FROM device_response_locks WHERE type='survey' AND target_id=?")->execute([$surveyId]);
            ...
            <form method="POST" action="comms_surveys.php?action=activate&id=<?= $surveyId ?>" style="display:inline;">
              ...
              <button type="submit" class="btn btn-green"
                      onclick="
                        ...
                        if (qt && qt.value.trim() !== '') {
                          return confirm('⚠️ You have an unsaved question.\n\nClick OK to activate anyway and lose it,\nor Cancel to go back and click + Add Question first.');
                        }
                        return confirm('Activate this survey now? It will be visible to all users.');
                      ">
                ▶ Activate Survey
              </button>
            </form>
