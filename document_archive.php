...
if (!empty($_SESSION['admin_id'])) {
    ...
    // Get resident email for levy lookup
...
if ($action === 'list') {
    ...
    if ($role === 'security') {
        // Security sees circulars only
        $where[] = "b.type = 'circular'";
    } elseif ($role === 'resident') {
        if ($resEmail) {
            // Resident sees circulars + their own levy notices
            $where[]  = "(b.type = 'circular' OR (b.type = 'levy' AND EXISTS (
                            SELECT 1 FROM document_send_log sl
            ...
            // No email on file — circulars only
            $where[] = "b.type = 'circular'";
        }
    }

    // Word search — title or notes
    if ($search) {
    ...
    <div class="container">
      ...
      <div class="card" style="padding:16px;">
        <form method="GET" style="display:flex;gap:8px;flex-wrap:wrap;align-items:flex-end;">
          ...
          <a href="document_archive.php" class="btn btn-secondary">Clear</a>
          <?php endif; ?>
        </form>
      </div>
      ...
      <div class="card">
        ...
        <div class="table-wrap"><table>
          ...
          <tr>
            ...
            <td>
              <a href="document_archive.php?action=view&id=<?= $d['id'] ?>"
...
if ($action === 'view') {
    ...
    if (!$id) { header('Location: document_archive.php'); exit; }

    $doc = db()->prepare("SELECT * FROM document_broadcasts WHERE id=? LIMIT 1");
    $doc->execute([$id]);
    $doc = $doc->fetch();
    if (!$doc) { header('Location: document_archive.php'); exit; }

    // Security can only view circulars
    if ($role === 'security' && $doc['type'] !== 'circular') {
        header('Location: document_archive.php'); exit;
    }

    // Resident: check they have access
    if ($role === 'resident' && $doc['type'] === 'levy') {
        if (!$resEmail) { header('Location: document_archive.php'); exit; }
        $check = db()->prepare(
            "SELECT id FROM document_send_log
             WHERE broadcast_id=? AND recipient_email=? AND status='sent' LIMIT 1"
        );
        $check->execute([$id, $resEmail]);
        if (!$check->fetch()) { header('Location: document_archive.php'); exit; }
    }

    // For resident levy — get their personal record
    $levyRecord = null;
    if ($role === 'resident' && $doc['type'] === 'levy' && $resEmail) {
        $lr = db()->prepare(
    ...
    $sendLog = [];
    if ($role === 'admin') {
        $sl = db()->prepare("SELECT * FROM document_send_log WHERE broadcast_id=? ORDER BY sent_at ASC");
        $sl->execute([$id]);
        $sendLog = $sl->fetchAll();
    }

    pageHeader(htmlspecialchars($doc['title']), $role);
    renderHeader('📄 ' . htmlspecialchars($doc['title']), 'document_archive.php');
    ?>
    <div class="container" style="max-width:700px;">
        ...
        <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:8px;">
          <div>
            ...
            <div style="font-size:.85rem;color:#888;">
              <?= $doc['type'] === 'circular' ? '📄 Circular' : '💰 Levy Notice' ?>
              &nbsp;|&nbsp;
          ...
          <?php if ($doc['type'] === 'circular' && $doc['filename']): ?>
      ...
      <?php if ($role === 'resident' && $doc['type'] === 'levy' && $levyRecord): ?>
      <div class="card">
